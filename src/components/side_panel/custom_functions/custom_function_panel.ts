...
import { CustomFunctionTabStore } from "./custom_function_tab_store";
...
export class CustomFunctionPanel extends Component<Props> {
  ...
  static components = { SidePanel, Section, ValidationMessages, Checkbox, TextInput, CodeEditor };
  static props = {
  ...
  popOutFunction() {
    this.saveFunction();
    if (!this.state.error) {
      this.customFunctionTabStore.openTab(this.state.tempName.toUpperCase());
    }
  }
