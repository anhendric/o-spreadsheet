...
export function isValidLocale(locale: any): locale is Locale {
  ...
  if (locale.formulaArrayRowSeparator === locale.formulaArgSeparator) {
    return false;
  }
  if (locale.formulaArrayRowSeparator === locale.decimalSeparator) {
    return false;
  }
...
function _localizeFormula(formula: string, fromLocale: Locale, toLocale: Locale) {
  ...
  for (const token of tokens) {
    if (token.type === "NUMBER") {
      ...
      localizedFormula += toLocale.formulaArrayRowSeparator;
    } else {
